 ...
 * $Id: org.transmart.SearchKeywordService.groovy 9178 2011-08-24 13:50:06Z mmcduffie $
 * $Revision: 9178 $
 *
 */
public class SearchKeywordService {
        ...
        for (filtercat in filtercats) {
            ...
            for (result in results) {
                if (filtercat.platformProperty) {
                    ...
                    if (!result?.codeName || !result?.bioDataUid?.uniqueId[0])
                    {println(filtercat);}
    ...
    def findSearchKeywords(category, term, max)	{
        ...
        for (result in results)	{
            ...
            else if (sk.searchKeyword.dataCategory.equals("STUDY"))
            {
                m.put("categoryId", sk.searchKeyword.dataCategory)
                m.put("id", sk.searchKeyword.bioDataId)
            }
            else if (sk.searchKeyword.dataCategory.equals("STUDY"))
            {
                m.put("categoryId", sk.searchKeyword.dataCategory)
    ...
    /** Searches for all keywords for a given term (like %il%) */
    def findSearchKeywordsSanofi(category, term, max)	{
        log.info "Finding matches for ${term} in ${category}"
