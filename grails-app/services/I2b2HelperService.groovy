...
import org.transmart.TransmartQueryDefinition
import org.transmart.TransmartQueryItem
import org.transmart.TransmartQueryPanel
...
class I2b2HelperService {
    ...
    def getConceptDistributionDataForValueConcept(String concept_key) {
        ...
        groovy.sql.Sql sql = new groovy.sql.Sql(dataSource);
        String concept_cd=getConceptCodeFromLongPath(concept_key);
        ArrayList<Double> values=new ArrayList<Double>();
    ...
    def getConceptDistributionDataForValueConceptFromCode(String concept_cd, String result_instance_id) {
        ArrayList<Double> values=new ArrayList<Double>();
        ArrayList<Double> returnvalues=new ArrayList<Double>(values.size());
        if (!result_instance_id || result_instance_id==""){
            log.debug("getConceptDistributionDataForValueConceptFromCode called with no result_instance_id");
            return returnvalues;
        }
     ...
     * Gets the count of the observations in the fact table for a concept and a subset
     */
    def Integer getObservationCountForConceptForSubsetSameEvent(String concept_key, TransmartQueryDefinition tQD) {

        log.trace("Getting observation count for concept:"+concept_key+" and instance:"+tQD.resultInstanceId);
        String fullname=concept_key.substring(concept_key.indexOf("\\",2), concept_key.length());

        StringBuilder sqlBuilder = new StringBuilder();
        List params = new ArrayList();

        def concept_cd = getConceptCodeFromPath(fullname);
        def link_type = getLinkTypeFromConceptCode(concept_cd);

        //append the root sql for the concept

        if (link_type == "S")
        {
            sqlBuilder.append("select count(distinct patient_num) from observation_fact");
        }
        else if (link_type == "E")
        {
            sqlBuilder.append("select count(distinct encounter_num) from observation_fact");
        }

        def isDistributionsHaveKey = tQD.distributions.containsKey(fullname);

        sqlBuilder.append(" WHERE patient_num IN (select distinct patient_num from qt_patient_set_collection where result_instance_id = ?)");
        params.add(tQD.resultInstanceId);

        if (isDistributionsHaveKey)
        {

            TransmartQueryItem qI = new TransmartQueryItem();
            qI.concept_cd = concept_cd;
            qI.linkType = link_type;
            qI.itemKey = concept_key;


            if (tQD.distributions[fullname].size() == 0) //There were no other panels with concepts
            {
                List<TransmartQueryItem> distSet = new ArrayList<TransmartQueryItem>();
                distSet.add(qI);
                sqlBuilder.append(" and encounter_num in (");
                sqlBuilder.append(buildCountSQLForDistribution(distSet , params));
                sqlBuilder.append(")");
            }
            else {

                for (distSet in tQD.distributions[fullname])
                {
                    distSet.add(0, qI);
                    sqlBuilder.append(" and encounter_num in (");
                    sqlBuilder.append(buildCountSQLForDistribution(distSet , params));
        ...
        def rootSQL = sqlBuilder.toString();
        groovy.sql.Sql sql = new groovy.sql.Sql(dataSource)
        sql.eachRow(rootSQL, params, {row ->
            i = row[0]
        })
        return i;
    ...
    def String buildCountSQLForDistribution(List<TransmartQueryItem> queryItems, List<String> params)
    {

        StringBuilder sqlBuilderSubject = new StringBuilder();
        StringBuilder sqlBuilderEncounter = new StringBuilder();
        StringBuilder finalSQLBuilder = new StringBuilder();
        Integer subjectCount = 0;
        Integer encounterCount = 0;

        List<String> encounterParams = new ArrayList<String>();
        List<String> subjectParams = new ArrayList<String>();

        for (tQI in queryItems)
        {
            ...
            if (tQI.linkType == "E")
            {
                suffix = " en" + encounterCount;

                if (encounterCount > 0)
                {
                    prefix = " inner join ";
                    suffix += " on en" + (encounterCount - 1) + ".encounter_num = en" + encounterCount + ".encounter_num"
                }

                sqlBuilderEncounter.append(prefix + "(" + buildCountSQLForConceptKey(tQI.fullName, encounterParams) + ")" + suffix) ;
                encounterCount++;
            }

            if (tQI.linkType == "S")
            {
                suffix = " su" + subjectCount;

                if (subjectCount > 0)
                {
                    prefix = " inner join ";
                    suffix += " on su" + (subjectCount - 1) + ".patient_num = su" + subjectCount + ".patient_num"
                }

                sqlBuilderSubject.append(prefix + "(" + buildCountSQLForConceptKey(tQI.fullName, subjectParams) + ")" + suffix) ;
                subjectCount++;
            }
        }

        params.addAll(encounterParams);
        params.addAll(subjectParams);

        String finalPrefix = "";
        String finalSuffix = "";
        String joinString = "";

        if (sqlBuilderEncounter.length() > 0)
        {
            def encounterPrefix = "(select en0.encounter_num encounter_num, en0.patient_num patient_num from (";
            sqlBuilderEncounter.insert(0, encounterPrefix);
            sqlBuilderEncounter.append(")) encounters");


            finalPrefix = "select encounters.encounter_num encounter_num from ";
        }

        if (sqlBuilderSubject.length() > 0)
        {
            def subjectPrefix = "(select su0.encounter_num encounter_num, su0.patient_num patient_num from (";
            sqlBuilderSubject.insert(0, subjectPrefix);
            sqlBuilderSubject.append(")) patients");

            if (sqlBuilderEncounter.length() == 0)
            {
                finalPrefix = "select patients.encounter_num encounter_num from ";
            }
        }

        if (sqlBuilderEncounter.length() > 0 && sqlBuilderSubject.length() > 0)
        {
            joinString = " inner join ";
            finalSuffix = " on encounters.patient_num = patients.patient_num";
        }

        finalSQLBuilder.append(finalPrefix);
        finalSQLBuilder.append(sqlBuilderEncounter.toString())
        finalSQLBuilder.append(joinString)
        finalSQLBuilder.append(sqlBuilderSubject.toString())
        finalSQLBuilder.append(finalSuffix);

        return finalSQLBuilder.toString();
    }

    private String buildCountSQLForConceptKey(String concept_key, List<String> params)
    {
        params.add(concept_key + "%");
        return "SELECT patient_num, encounter_num FROM observation_fact WHERE concept_cd IN " +
                "(SELECT concept_cd FROM concept_dimension WHERE concept_path LIKE ?)";

    }

    def HashMap<String, List<List<TransmartQueryItem>>> loadDistributions(TransmartQueryDefinition tQD)
    {

        def conceptDistributions = new HashMap<String, List<List<TransmartQueryItem>>>();
        def panels = tQD.panels;

        //Create an array with the counts and number of combinations from each panel
        Integer[] cpArray = new Integer[panels.size()];

        for (Integer i = 0; i < panels.size(); i++)
        {
            cpArray[i] = panels.get(i).items.size();
        }

        //Loop through each panel one concept at a time to create each concepts distribution set
        for (Integer currentPIndex = 0; currentPIndex < panels.size(); currentPIndex++) {

            //Load the other panels into an array to iterate for creating the distributions for this concept
            TransmartQueryPanel[] otherPanels;

            if (panels.size() > 0) {
                otherPanels = new TransmartQueryPanel[panels.size() - 1];
                Integer otherPanelIndex = 0;
                for (Integer i = 0; i < panels.size(); i++)
                {
                    if (i != currentPIndex)   {
                        otherPanels[otherPanelIndex] = panels.get(i);
                        otherPanelIndex++;
                    }
                }
            }

            //Loop through each concept in the panel using the other panels to create the concept distributions
            for (Integer currentCIndex = 0; currentCIndex < panels.get(currentPIndex).items.size(); currentCIndex++) {

                List<List<TransmartQueryItem>> cSet = new ArrayList<ArrayList<TransmartQueryItem>>();

                if (otherPanels) {

                    Boolean doneCreatingDistributions = false;

                    Integer[] pCountAry = new Integer[otherPanels.length];  //Array to hold other panel counts
                    Integer[] pCurrentIndexAry = new Integer[otherPanels.length];  //Array to hold other panel indexes when building distribution

                    //Fill those arrays
                    for (Integer i = 0; i < otherPanels.length; i++)
                    {
                        pCountAry[i] = otherPanels[i].items.size();
                        pCurrentIndexAry[i] = 0;
                    }

                    //Create the distributions
                    while (! doneCreatingDistributions)
                    {
                        ArrayList<TransmartQueryItem> cDistribution = new ArrayList<TransmartQueryItem>();
                        //Add all the concepts using the index array as a pointer to each panels concept
                        for (Integer i = 0; i < pCurrentIndexAry.length; i++)
                        {
                            //Add the concept to the distribution list only if its not the same concept as the current concept
                            if (otherPanels[i].items.get(pCurrentIndexAry[i]).itemKey != panels.get(currentPIndex).items.get(currentCIndex).itemKey)
                            {
                                cDistribution.add(otherPanels[i].items.get(pCurrentIndexAry[i]));
                            }
                        }

                        //Add the distribution to the concepts distribution set only if there isn't an existing set that matches
                        if (! tQD.isSetInDistribution(cDistribution, cSet))
                        {
                            cSet.add(cDistribution)
                        };

                        //Roll the indexes so we can add the next set of concepts
                        Boolean doneRolling = false;
                        Integer activePanelIndex = otherPanels.length - 1;
                        while (! doneRolling)
                        {
                            pCurrentIndexAry[activePanelIndex]++;

                            //Did this panel index roll over?
                            if (pCurrentIndexAry[activePanelIndex] == pCountAry[activePanelIndex]){

                                //We are done if the final panel rolled over
                                if (activePanelIndex == 0)
                                {
                                    doneCreatingDistributions = true;
                                    doneRolling = true;
                                }
                                else  //Otherwise roll the indexes, if the current panel rolled over set the next panel to roll its index
                                {
                                    pCurrentIndexAry[activePanelIndex] = 0;
                                    activePanelIndex--;
                                    doneRolling = false;
                                }
                            }
                            ...
                            {
                                doneRolling = true;  //We are done rolling for now if this panel didn't roll over
                            }
                        }
                    }
                }

                def key = panels.get(currentPIndex).items.get(currentCIndex).fullName;

                //Add this concepts set of distributions to Hashmap or add them to existing distributions list if the concept has been previously processed
                if (conceptDistributions.containsKey(key))
                {
                    for (dl in cSet)
                    {
                        //Don't add a duplicate set
                        if (!tQD.isSetInDistribution(dl, conceptDistributions[key]))
                        {
                            conceptDistributions[key].add(dl);
                        }
                    }
                }
                else
                {
                    conceptDistributions.put(key, cSet);
                }
            }
        }
    ...
    def getChildrenWithAccessForUser(String concept_key, AuthUser user) {
        ...
        if(level==-1 && !admin) //only check on first level of nodes and im not an admin
        {
            ...
            s2.append("SELECT DISTINCT s FROM org.transmart.searchapp.SecureObjectPath s WHERE s.conceptPath LIKE'").append(path).append("%'")
            ...
            StringBuilder s = new StringBuilder();
            s.append("SELECT DISTINCT ausa.accessLevel, sop.conceptPath FROM org.transmart.searchapp.AuthUserSecureAccess ausa JOIN ausa.accessLevel JOIN ausa.authUser au JOIN ausa.secureObject.conceptPaths sop ");
    ...
    def getConceptPathAccessForUser(List<String> paths, AuthUser user) {
        ...
        if(!admin) //level of nodes and im not an admin
        {
            ...
            StringBuilder s2=new StringBuilder();
            s2.append("SELECT DISTINCT s FROM org.transmart.searchapp.SecureObjectPath s WHERE s.conceptPath IN (:ids )");
    ...
    def getConceptPathAccessCascadeForUser(List<String> paths, AuthUser user) {
        ...
        if(!admin) //level of nodes and im not an admin
        {
            ...
            //3) get the secure paths that are in the list and secure them for later unlocking if necessary
            StringBuilder s2=new StringBuilder();
            s2.append("SELECT DISTINCT s FROM org.transmart.searchapp.SecureObjectPath s");
            ...
            //4) get the access levels this user has and unlock the locked resources available to him
            StringBuilder s = new StringBuilder();
            s.append("SELECT DISTINCT ausa.accessLevel, sop.conceptPath FROM org.transmart.searchapp.AuthUserSecureAccess ausa JOIN ausa.accessLevel JOIN ausa.secureObject.conceptPaths sop ");
    ...
    def getSecureTokensWithAccessForUser(user) {
        StringBuilder s = new StringBuilder();
        s.append("SELECT DISTINCT ausa.accessLevel, so.bioDataUniqueId FROM org.transmart.searchapp.AuthUserSecureAccess ausa JOIN ausa.accessLevel JOIN ausa.secureObject so ")
