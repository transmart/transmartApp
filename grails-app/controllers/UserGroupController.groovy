...
class UserGroupController {
    ...
    def ajaxGetUserSearchBoxData=
    {
		...
		def userdata=[];
		 def users=AuthUser.executeQuery("from org.transmart.searchapp.AuthUser p where upper(p.name) like upper ('%"+searchText+"%') order by p.name");
			users.each{user ->
	 ...
	 def ajaxGetUsersAndGroupsSearchBoxData=
	    {
	    	String searchText = request.getParameter("query");
			def userdata=[];
			 def users=Principal.executeQuery("from org.transmart.searchapp.Principal p where upper(p.name) like upper ('%"+searchText+"%') order by p.name");
				users.each{user ->
