...
class DataExportService {
    ...
    def exportData(jobDataMap) {
        ...
        //This will determine the format of the final clinical data output file. For now we deal with regular text, or SAS which requires some processing to make it compatible.
        List<String> clinicalFileFormat = new ArrayList<String>();
        ...
        try {
            subsets.each { subset ->
                ...
                if (null != selectedFilesList && !selectedFilesList.isEmpty()) {
                    ...
                    if ((jobDataMap.get("codeType") != "Modifier") && (null != resultInstanceIdMap[subset] && !resultInstanceIdMap[subset].isEmpty())) {
                    ...
                    if(jobDataMap.get("codeType") == "Modifier")
                    {
                        studyDir = new File(jobTmpDirectory, subset)
                        studyDir.mkdir()
                    }
                    ...
                    if(
                            (
                            null != resultInstanceIdMap[subset] && !resultInstanceIdMap[subset].isEmpty()
                            )
                                    ||
                                    (
                                    jobDataMap.get("codeType") == "Modifier" && subset !="subset2"
                                    )
                    )
                    {
                        ...
                        selectedFilesList.each() { selectedFile ->
                            ...
                            //This needs to allow for both file types at the same time.
                            if (StringUtils.equalsIgnoreCase(selectedFile, "CLINICAL.TXT")) {
                                clinicalFileFormat.add("TAB-TEXT")
                                writeClinicalData = true
                            }

                            if (StringUtils.equalsIgnoreCase(selectedFile, "CLINICAL.SAS.TXT")) {
                                clinicalFileFormat.add("SAS")
                                writeClinicalData = true
                            }

                            if (StringUtils.equalsIgnoreCase(selectedFile, "CLINICAL.ENCOUNTER.TXT")) {
                                clinicalFileFormat.add("ENCOUNTER")
                                writeClinicalData = true
                            }
                            ...
                            switch (selectedFile)
                            {
                                ...
                                case "IGV.VCF":
                                    ...
                                    def selectedGenesAndId = jobDataMap.get("selectedGenesAndId")
                                    ...
                                    println("selectedGenesAndId:" + selectedGenesAndId)
                                    ...
                                    def webRootName = org.codehaus.groovy.grails.web.context.ServletContextHolder.getServletContext().getRealPath("/")
                                    ...
                                    if('subset2'==subset)
                                        prefix = "S2"
                                    vcfDataService.getDataAsFile(outputDir, jobDataMap.get("jobName"), null, resultInstanceIdMap[subset], selectedSNPs, selectedGenes, selectedGenesAndId, chromosomes, prefix);
                                    break;
                            }
                        }
                    }
                    ...
                    if (writeClinicalData) {
                        ...
                        if(jobDataMap.get("codeType") == "Modifier")
                        {
                            ...
                            String[] modifierCodeList = jobDataMap.get("modifier_cds");

                            //Make this blank instead of null if we don't find it.
                            if(modifierCodeList == null)	modifierCodeList = []

                            retVal = clinicalDataService.getDataCrossTrial(studyList, studyDir, "clinical.i2b2trans", jobDataMap.get("jobName"),
                                    resultInstanceIdMap[subset], modifierCodeList, selectedFilesList, pivotData, filterHighLevelConcepts,
                                    snpFilesMap, subset, filesDoneMap, platformsList, parentConceptCodeList as String[], includeConceptContext, clinicalFileFormat)
                        }
                        else
                        {
                            //This is a list of concept codes that we use to filter the result instance id results.
                            ...
                            retVal = clinicalDataService.getData(studyList, studyDir, "clinical.i2b2trans", jobDataMap.get("jobName"),
