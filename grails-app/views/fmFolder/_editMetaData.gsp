                ...
                <g:if test="${amTagItem.tagItemType == 'FIXED'  && amTagItem.tagItemAttr!=null?bioDataObject?.hasProperty(amTagItem.tagItemAttr):false}" >
                	...
                	<g:else>
	                	<g:if test="${amTagItem.tagItemSubtype == 'PICKLIST'}">
		                	...
		                	name="${amTagItem.tagItemAttr}" value="${fieldValue(bean:bioDataObject,field:amTagItem.tagItemAttr)}" optionKey="${{it.bioDataUid.uniqueId[0]}}" optionValue="codeName"  noSelection="['':'-Select One-']" />	
		                	<!-- Previous value: ${fieldValue(bean:bioDataObject,field:amTagItem.tagItemAttr)} -->
						</g:if>
