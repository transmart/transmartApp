...
function addSpace(str, words, startIndex)
{
    for (var i = startIndex; i < words.length; i++)
    {
        var pos = str.indexOf(" ", 0);
        if (pos != -1)
            return addSpace(str.substring(0, pos), words, i) + " " + addSpace(str.substring(pos + 1), words, i);
        else if (pos == -1 && isEasternLanguage(str))
        {
            pos = str.indexOf(words[i], 0);
            if (pos != -1)
            {
                var left = addSpace(str.substring(0, pos), words, i);
                var right = addSpace(str.substring(pos + words[i].length), words, i);
                if (left && left != "\"")
                    left += " ";
                if (right && right != "\"")
                    right = " " + right;
...
function addSpacesBetweenWords(elem)
{
    ...
    var res = addSpace(query, words, 0);
    elem.value = res;
    return res;
}
...
function d2hSplitRequest(request)
{
    var arr = new Array();
    var regExp = /"(.)*"/gi;
    var obj = regExp.exec(request);
    var i = 0;
    while (obj && obj.index != -1)
    {
        arr[i] = obj[0].substring(1, obj[0].length-1);
        if (isEasternLanguage(arr[i]))
            arr[i] = arr[i].replace(/\s+/gi, "?");
        i += 2;
        request = request.replace(obj[0], "");
        obj = regExp.exec(request);
    }
    var mainWindow = d2hGetMainWindow();    
