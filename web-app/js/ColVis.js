...
ColVis.prototype = {
    ...
    "fnSaveVis": function ()
    {
        ...
        {
            columnVis[i] = this.s.dt.aoColumns[i].bVisible;
        }

        window.columnVis = columnVis;

    },

    "fnApplyVis": function ()
    {
        for ( var i=0, iLen=window.columnVis.length ; i<iLen ; i++ )
        {
            this.s.dt.oInstance.fnSetColumnVis( i, window.columnVis[i], false );
        }
        this._fnAdjustOpenRows();
	...
	"_fnDomBaseButton": function ( text )
	{
		...
		$(nButton).bind( sEvent, function (e) {
			...
			//Don't do anything if the animation is not finished
			if (that.bShowing || that.bHiding) return;
			
			//This is needed because for some reason in IE the catcher event is not being fired, only the button event.
			if ( that.s.hidden || that.dom.collection == null  ) {
	...
	"_fnDomCatcher": function ()
	{
		...
		nCatcher.style.opacity = "0";
		nCatcher.style.filter = "alpha(opacity=0)";
		$(nCatcher).html("&nbsp;");
 ...
 *  @method  ColVis.fnSaveVis
 *  @static
 *  @param   object oTable DataTable instance to consider - optional
 *  @returns void
 */
ColVis.fnSaveVis = function ( oTable )
{
    var nTable = null;
    if ( typeof oTable != 'undefined' )
    {
...
/**
     ...
     *  @method  ColVis.fnApplyVis
     *  @static
     *  @param   object oTable DataTable instance to consider - optional
     *  @returns void
     */
ColVis.fnApplyVis = function ( oTable )
{
    var nTable = null;
    if ( typeof oTable != 'undefined' )
    {
    ...
    for ( var i=0, iLen=ColVis.aInstances.length ; i<iLen ; i++ )
    {
        if ( typeof oTable == 'undefined' || nTable == ColVis.aInstances[i].s.dt.nTable )
        {
            ColVis.aInstances[i].fnApplyVis();
        }
    }
};
