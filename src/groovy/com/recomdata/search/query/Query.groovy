...
public class Query {
	...
	def createExpandBioMarkerSubQuery(ids){
		...
		// s.append("SELECT DISTINCT marker.id FROM org.transmartproject.biomart.BioMarker marker ")
	...
	def createExpandBioMarkerCondition(String markerAlias, GlobalFilter gfilter){
		...
		if(!gfilter.getGenePathwayFilters().isEmpty()){
			...
			s.append(markerAlias).append(".id IN (")
			s.append("SELECT DISTINCT bmcmv.assoBioMarkerId FROM org.transmartproject.biomart.BioMarkerCorrelationMV bmcmv ");
