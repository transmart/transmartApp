...
class ChartController {
   ...
   def conceptPatientCount = {
    ...
    def conceptDistribution = {
        ...
        String codeKey = params.concept_key;
        String codePath = params.concept_path;
        String codeType = params.codeType;

        def al = new AccessLog(username:springSecurityService.getPrincipal().username, event:"DatasetExplorer-Set Value Concept Histogram", eventmessage:"Concept:"+codeKey, accesstime:new java.util.Date())
        al.save()

        def codeName
        double[] values

        //Depending on the code type we retrieve the data differently.
        if(codeType == "Modifier")
        {
            codeName = i2b2HelperService.getShortNameFromKey(codePath);
            values = i2b2ModifierHelperService.getModifierDistributionDataForValueModifier([codeKey] as Set);
        }
        else
        {
            codeName = i2b2HelperService.getShortNameFromKey(codeKey);
            values = i2b2HelperService.getConceptDistributionDataForValueConcept(codeKey);
        }
        ...
        //pw.write("<html><body>");
        pw.write("<img src='"+graphURL+"' width=245 height=180 border=0 usemap='#"+filename+"' alt='Summary Stats Graph'>");
    ...
    def conceptDistributionForSubset = {
        ...
        def al = new AccessLog(username:springSecurityService.getPrincipal().username, event:"DatasetExplorer-Set Value Concept Histogram for subset", eventmessage:"Concept:"+codeKey, accesstime:new java.util.Date())
        al.save()
        ...
        if(result_instance_id1!="" && result_instance_id1 != null)
        {
            ...
            if(codeType == "Modifier")
            {
                ...
                modifierObject["modifierCode"] = codeKey;
                modifierObject["level"] = "";
                modifierObject["oktousevalues"] = "";
                modifierObject["name"] = "";
                modifierObject["inOutCode"] = "";

                //The object we return in this instance is a map of the values. We always retrieve the "All Studies" key.
                def returnFromDistributionGather = i2b2ModifierHelperService.getModifierDistributionDataForValueModifierWithResultInstanceId(modifierObject, result_instance_id1)

                if(returnFromDistributionGather != null)
                {
                    values2 = returnFromDistributionGather['All Studies'];
                }
                else
                {
                    values2 = new ArrayList<Double>(0)
                }
            ...
            else
            {
                ...
                values2 = i2b2HelperService.getConceptDistributionDataForValueConcept(codeKey, result_instance_id1);
                values = i2b2HelperService.getConceptDistributionDataForValueConcept(codeKey);
            }
            ...
            JFreeChart chart2 = ChartFactory.createHistogram(
                    "Histogram of "+codeName +" for subset",
                    null,
                    "Count",
                    dataset2,
                    PlotOrientation.VERTICAL,
            ...
            PrintWriter pw=new PrintWriter(response.getOutputStream());
            pw.write("<img src='"+graphURL2+"' width=245 height=180 border=0 usemap='#"+filename2+"' alt='Summary Stats Graph'>");
  ...
  */
  ...
  def basicStatistics = {
		...
		if(s1){
            def regionFlag1 = regionParams?.regionParams1 ? regionParams.regionParams1[0] : null;
            i2b2HelperService.renderQueryDefinitionTable(result_instance_id1, "Query Summary for Subset 1", pw, regionFlag1);
        }
		pw.write("</td><td align='center'>");
		if(s2){
            def regionFlag2 = regionParams?.regionParams2 ? regionParams.regionParams2[0] : null;
            i2b2HelperService.renderQueryDefinitionTable(result_instance_id2, "Query Summary for Subset 2", pw, regionFlag2);
        }
		pw.write("</tr>");
...
def analysisGrid = {
    ...
    def analysisGridModifier =
        {
            ...
            modifierObject["modifierCode"] = params.modifierCode;
            modifierObject["level"] = params.level;
            modifierObject["oktousevalues"] = params.oktousevalues;
            modifierObject["name"] = params.name;
            modifierObject["inOutCode"] = params.inOutCode;
            ...
            def al = new AccessLog(username:springSecurityService.getPrincipal().username, event:"DatasetExplorer-Grid Analysis Drag", eventmessage:"RID1:"+result_instance_id1+" RID2:"+result_instance_id2+" Modifier : "+modifierObject["name"], accesstime:new java.util.Date())
            al.save()
            ...
            if(table==null)
            {
                ...
                if(s2)
                {
                    gridViewService.addAllPatientDemographicDataForSubsetToTable(table, result_instance_id2, "subset2");}
            }
            ...
            {
                addLeafModifierToTable(result_instance_id1, result_instance_id2, log, modifierObject, table, s1, s2)
            }
            else
            {
                addFoldeModifierToTable(result_instance_id1, result_instance_id2, log, modifierObject, table)
            }
    ...
    private addLeafModifierToTable(String result_instance_id1, String result_instance_id2, org.apache.commons.logging.Log log, modifierObject, ExportTableNew table, boolean s1, boolean s2)
    {
        if(result_instance_id1){i2b2ModifierHelperService.addModifierDataToTable(table, modifierObject, result_instance_id1);}
        if(result_instance_id2){i2b2ModifierHelperService.addModifierDataToTable(table, modifierObject, result_instance_id2);}
    }

    private addFoldeModifierToTable(String result_instance_id1, String result_instance_id2, org.apache.commons.logging.Log log, modifierObject, ExportTableNew table) {
        //Get all the child modifiers for this folder.
        def childPathsFromModifier = i2b2ModifierHelperService.getChildModifiersFromParentKey(modifierObject["name"], false, modifierObject["inOutCode"]);

        for(childModifier in childPathsFromModifier)
        {
            if(result_instance_id1){i2b2ModifierHelperService.addModifierFolderDataToTable(table, modifierObject, childModifier, result_instance_id1);}
            if(result_instance_id2){i2b2ModifierHelperService.addModifierFolderDataToTable(table, modifierObject, childModifier, result_instance_id2);}
        }
    }
