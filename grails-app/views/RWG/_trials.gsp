...
<div class="search-results-table">
    <g:each in="${trials.entrySet()}" status="ti" var="trialresult">        
        <div class="${ (ti % 2) == 0 ? 'result-trial-odd' : 'result-trial-even'}" id="TrialDet_${trialresult.key.id}_anchor">                            
            <a href="#" onclick="javascript:showDetailDialog('${createLink(controller:'trial',action:'expDetail',id:trialresult.key.id)}', '${trialresult.key.trialNumber} Details', 650);">
               ...
               <div id="${trialresult.key.trialNumber}_detail"></div>
        </div> 
    </g:each>
</div>