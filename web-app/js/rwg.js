...
function loadBoxPlotData(analysisID, probeID)	{	
	...
	rwgAJAXManager.add({
		...
		success: function(response) {
			setActiveProbe(analysisID, probeID);
			//var analysisData = response[analysisID];
...
function loadBoxPlotCTA(gene_id)	{	
	...
	rwgAJAXManager.add({
		...
		success: function(response) {
			...
			// create a div for each boxplot  
/*			for (var i=0; i<selectedAnalyses.length; i++)
			{
				var id = selectedAnalyses[i].id;
				var divID = "xtBoxPlot" + id;
				jQuery("#xtBoxplot").append(jQuery("<div>").attr("id", divID).html(selectedAnalyses[i].id));
				
				drawBoxPlotD3(divID, response[id], id, false, selectedAnalyses[i].title);
			}
*/			
