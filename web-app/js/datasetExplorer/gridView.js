...
//      Calculate cell data types and extra class names which affect formatting
        ...
        for (var i = 0; i < cm.getColumnCount(); i++) {
            if (includeHidden || !cm.isHidden(i)) {
                ...
                var tooltipHasExtraInformation = !!cm.getColumnTooltip(i) && cm.getColumnHeader(i) != cm.getColumnTooltip(i);
                if (tooltipHasExtraInformation) {
                    headerXml += '<ss:Comment><ss:Data>' + cm.getColumnTooltip(i) + '</ss:Data></ss:Comment>';
                }
                headerXml += '<ss:NamedCell ss:Name="Print_Titles" /></ss:Cell>';
...
//      Generate worksheet header details.
        var t = '<ss:Worksheet ss:Name="' + this.title + '">' +
            ...
            '</html:Font></html:U></html:B> Exported from the grid view</ss:Data><ss:NamedCell ss:Name="Print_Titles" />' +
            '</ss:Cell>' +
            '</ss:Row>' +
